...
class ParseArea {
  ...
  static init() {
    for (const code in _area.default.province_list) {
      const province = _area.default.province_list[code];
    ...
    for (const code in _area.default.city_list) {
      const city = _area.default.city_list[code];
      if (city.length > 2) {
        ParseArea.CityShort[code] = CityKeys.reduce((v, key) => v.replace(key, ""), city);
      }
    }
    for (const code in _area.default.area_list) {
      let area = _area.default.area_list[code];
  ...
  static parseByProvince(addressBase) {
    const province_list = _area.default.province_list;
    const results = [];
    const result = {
      province: "",
      city: "",
  ...
  static parse_city_by_province(address, result) {
    const cityList = _utils.default.getTargetAreaListByCode("city", result.code);
    const _result = {
      city: "",
      code: "",
      index: -1,
  ...
  static parse_area_by_city(address, result) {
    const areaList = _utils.default.getTargetAreaListByCode("area", result.code);
    const _result = {
      area: "",
      code: "",
      index: -1,
    ...
    for (const area of areaList) {
      ...
      if (shortArea) {
        const {
          index: _index,
          matchName
        } = _utils.default.shortIndexOf(address, shortArea, area.name);
        index = _index;
        shortArea = matchName;
      }
      if (index === -1) {
        const aliasMatch = ParseArea.matchDevelopmentAreaAlias(address, area.name);
  ...
  static matchDevelopmentAreaAlias(address, areaName) {
    ...
    return ParseArea.DevelopmentAreaAliases.reduce((result, alias) => {
      const index = address.indexOf(alias);
      if (index > -1 && (result.index === -1 || index < result.index || index === result.index && alias.length > result.matchName.length)) {
        result.index = index;
  ...
  static parse_area_by_province(address, result) {
    ...
    for (const area of areaList) {
      ...
      const areaLength = shortArea ? shortArea.length : area.name.length;
      if (index > -1 && index < 6) {
        const [city] = _utils.default.getTargetAreaListByCode("city", area.code, true);
        result.city = city.name;
  ...
  static parseByCity(addressBase) {
    const city_list = _area.default.city_list;
    const results = [];
    const result = {
      province: "",
      city: "",
    ...
    for (const code in city_list) {
      ...
      if (index > -1) {
        const [province] = _utils.default.getTargetAreaListByCode("province", code, true);
  ...
  static parseByArea(addressBase) {
    const area_list = _area.default.area_list;
    const results = [];
    const result = {
      province: "",
      city: "",
    ...
    for (const code in area_list) {
      ...
      if (shortArea) {
        const {
          index: _index,
          matchName
        } = _utils.default.shortIndexOf(address, shortArea, area);
        index = _index;
        shortArea = matchName;
      }
      const areaLength = shortArea ? shortArea.length : area.length;
      if (index > -1) {
        const [province, city] = _utils.default.getTargetAreaListByCode("province", code, true);
        result.province = province.name;
        result.city = city.name;
        result.area = area;
        result.code = code;
...
var _default = exports.default = ParseArea;