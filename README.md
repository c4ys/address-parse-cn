...
可以用命令行手工查看解析结果，默认等同于 `AddressParse.parse(address, true)`：

````
pnpm --silent parse "广东东莞市大朗镇测试路288号测试大厦22楼测试律师事务所"
````

只查看第一条结果：

````
pnpm --silent parse --first "广东东莞市大朗镇测试路288号测试大厦22楼测试律师事务所"
````
