...
class ParseArea {
  ...
  static init() {
    for (const code in AREA.province_list) {
      ...
      ParseArea.ProvinceShort[code] = ProvinceKeys.reduce(
        (v, key) => v.replace(key, ""),
        province
      );
    ...
    for (const code in AREA.city_list) {
      ...
      if (city.length > 2) {
        ParseArea.CityShort[code] = CityKeys.reduce(
          (v, key) => v.replace(key, ""),
          city
        );
      }
        ...
        if (
          ...
          if (result.city) {
            ...
            if (result.area && _address.includes(result.area)) {
              ...
              result.area &&
              _address.includes(result.area.substr(0, 2))
            ) {
              result.__parse += 0.5;
            }
          }
    ...
    for (const code in province_list) {
      ...
      if (index > -1) {
        ...
        if (result.city) {
          ...
          if (
            index > 4 &&
            ParseArea.ProvinceShortList.some((shortProvince) =>
              result.name.includes(shortProvince)
            )
          ) {
  ...
  static parse_area_by_city(address, result) {
    ...
    for (const area of areaList) {
      ...
      if (shortArea) {
        const { index: _index, matchName } = Utils.shortIndexOf(
          address,
          shortArea,
          area.name
        );
  ...
  static parseByCity(addressBase) {
    ...
    for (const code in city_list) {
      ...
      if (index > -1) {
        const [province] = Utils.getTargetAreaListByCode(
          "province",
          code,
          true
        );
  ...
  static parseByArea(addressBase) {
    ...
    for (const code in area_list) {
      ...
      if (index > -1) {
        const [province, city] = Utils.getTargetAreaListByCode(
          "province",
          code,
          true
        );
        ...
        if (shortArea && address.charAt(index + areaLength) === "县")
          index += 1;
        address = address.substr(index + areaLength);
